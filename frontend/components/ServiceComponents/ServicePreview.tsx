...
import { Clock, IndianRupee, MapPin, CalendarCheck, Info } from "lucide-react";
import { Badge } from "@/components/ui/badge";
...
export function ServicePreview({ data }: PreviewProps) {
  return (
    <div className="flex flex-col h-full border-2 border-dashed border-border rounded-[2rem] bg-muted/40 p-6 space-y-6">
      {/* Header */}
      <div className="flex items-center justify-between">
      ...
      <div className="bg-card rounded-2xl border border-border shadow-sm p-6 space-y-4 transition-all">
        <div className="flex justify-between items-start gap-4">
          ...
          <Badge
            variant="secondary"
            className="capitalize shrink-0 bg-primary/10 text-primary border-none"
          >
            {data.serviceType.toLowerCase()}
          </Badge>
        </div>
        ...
        {/* Location */}
        {data.serviceType === "OFFLINE" && (
          <div className="flex items-center gap-2 text-xs text-muted-foreground bg-muted p-2 rounded-lg truncate">
            <MapPin className="w-3.5 h-3.5 shrink-0" />
            {data.locationAddress || "No address provided yet"}
          </div>
        )}
        ...
        <button
          type="button"
          className="w-full py-3.5 bg-primary text-primary-foreground rounded-xl text-sm font-bold flex items-center justify-center gap-2 shadow-md hover:opacity-90 transition"
        >
          <CalendarCheck className="w-4 h-4" />
          Confirm Booking
        </button>
